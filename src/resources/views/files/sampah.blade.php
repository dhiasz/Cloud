                    ...
                    >
                        ...
                        <div class="mt-1 flex gap-1 flex-wrap justify-center">
                            @if(in_array($ext, ['jpg','jpeg','png','gif','pdf','mp4','mkv','mov','avi']))
                                <!-- optional: preview could be implemented to read from .trash if desired -->
                                <a href="#" class="bg-gray-500 text-white px-2 py-1 rounded hover:bg-gray-600 text-xs">Preview</a>
                            @endif

                            <form action="{{ route('files.restore', ['path' => $relativeFilePath]) }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" class="bg-green-500 text-white px-2 py-1 rounded hover:bg-green-600 text-xs">Restore</button>
                            </form>

                            <form action="{{ route('files.trash.delete', ['path' => $relativeFilePath]) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="bg-red-500 text-white px-2 py-1 rounded hover:bg-red-600 text-xs">Hapus Permanen</button>
                            </form>
